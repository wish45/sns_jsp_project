...
<jsp:useBean id="member" class="mysns.member.MemberVO"/>
...
<%
	...
	if(action.equals("new")){
		if(mdao.addMember(member)){
			out.println("<script>alert('정상적으로 등록 되었씁니당.로그인 ㄱㄱ ');opener.window.location.reload();window.close();</script>");
		}
		else{
			out.println("<script>alert('같은 아이디 존재.');history.go(-1);</script>");
		}
	}
